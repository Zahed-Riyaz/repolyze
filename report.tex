...
\begin{longtable}{L{3.4cm} L{1.5cm} L{8.6cm}}
    ...
    \texttt{manifest.json} & Config &
        Declares extension metadata, all permissions (\texttt{sidePanel}, \texttt{tabs}, \texttt{storage}), content-script injection, and host permissions for GitHub + five AI provider APIs. \\
    \addlinespace
    ...
    \texttt{content.js} & Content Script &
        Runs inside every GitHub tab; parses \texttt{owner/repo} from the URL path and broadcasts it via \texttt{chrome.runtime.sendMessage}. \\
    \addlinespace
    ...
    \texttt{popup.html} & Unused &
        Empty placeholder; the extension uses a side panel instead of a browser-action popup. \\
    \addlinespace

    \texttt{popup.js} & Unused &
        Empty placeholder; no popup logic is needed. \\
...
\begin{longtable}{L{4cm} L{2cm} L{7.5cm}}
    ...
    \addlinespace
    Root directory listing & N/A & File names only; reveals project structure. \\
\end{longtable}

Content is returned as base64-encoded strings from the GitHub API and decoded with \texttt{atob()}. The entire result is \textbf{cached in the session-scoped \texttt{repoCache}} object so subsequent messages in the same session reuse the context without additional API calls.
...
\begin{longtable}{L{2.8cm} L{4.4cm} L{2.8cm} L{3.5cm}}
    ...
    Gemini  & \texttt{gemini-2.0-flash}           & SSE (\texttt{?alt=sse}) & 1M tokens \\
    Groq    & \texttt{llama-3.3-70b-versatile}    & OpenAI SSE              & 128k tokens \\
    OpenAI  & \texttt{gpt-4o-mini}                & OpenAI SSE              & 128k tokens \\
    Anthropic & \texttt{claude-3-5-haiku-20241022} & Anthropic SSE          & 200k tokens \\
